...
| Q13 | LLM | Qwen 3 via DashScope（`qwen3-plus` + `qwen3-max`） | 单场 ~¥0.18，中文最强的便宜选择 |
...
- [ ] 添加 `.env.local` 示例：`DASHSCOPE_API_KEY`、`MODEL_ADVISOR=qwen3-plus`、`MODEL_SYNTHESIZER=qwen3-max`
...
- [ ] `api/intake-clarify.ts`：主持人判断+追问 prompt，调 qwen3-plus，返回 JSON
- [ ] `api/advisor/[id].ts`：读入选中军师的 SKILL.md，拼接 prompt（含 meta 块要求），调 qwen3-plus 流式，SSE 输出（含末尾 `<meta>` 块）
...
2. **上下文拼接长度**：所有军师 SKILL.md 都塞进去的话，一个军师发言时输入 token ~20-30k，5 位军师的议程最后一位需要看到前 4 位的回复，加起来可能 ~50k。qwen3-plus 1M 上限绰绰有余，但实际延迟和成本会比第一位高 2-3 倍。
...
| 2026-04-22 | v2 修订 | 删除 `C` 段（心智模型跨域可用，场景限制扼杀跨域洞察价值）；改用 Qwen 3 系列（`qwen3-plus` + `qwen3-max`）；成本估算从 ¥2-3/场 降到 ¥0.18/场 |
