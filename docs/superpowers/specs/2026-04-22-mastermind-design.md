...
| Q13 | LLM | Qwen 3 via DashScope（`qwen3.6-max-preview` + `qwen3.6-max-preview`） | 单场 ~¥0.18，中文最强的便宜选择 |
...
└────────────┬─────────────────────────────────┘
             ...
             ▼
     DashScope · Qwen 3 系列
     ├─ qwen3.6-max-preview   (军师发言，1M context)
     └─ qwen3.6-max-preview    (Analyst 思维分析员，262k context)
...
**模型**：`qwen3.6-max-preview`（1M 上下文绰绰有余）
...
**模型**：`qwen3.6-max-preview`（校验 + 归纳质量是瓶颈，值得最强模型）
...
- [ ] 添加 `.env.local` 示例：`DASHSCOPE_API_KEY`、`MODEL_ADVISOR=qwen3.6-max-preview`、`MODEL_SYNTHESIZER=qwen3.6-max-preview`
...
- [ ] `api/intake-clarify.ts`：主持人判断+追问 prompt，调 qwen3.6-max-preview，返回 JSON
- [ ] `api/advisor/[id].ts`：读入选中军师的 SKILL.md，拼接 prompt（含 meta 块要求），调 qwen3.6-max-preview 流式，SSE 输出（含末尾 `<meta>` 块）
- [ ] `api/analyze.ts`：拼接 Analyst prompt（校验模式），调 qwen3.6-max-preview 流式，SSE 输出 per-character 决策卡
...
2. **上下文拼接长度**：所有军师 SKILL.md 都塞进去的话，一个军师发言时输入 token ~20-30k，5 位军师的议程最后一位需要看到前 4 位的回复，加起来可能 ~50k。qwen3.6-max-preview 1M 上限绰绰有余，但实际延迟和成本会比第一位高 2-3 倍。
...
| 2026-04-22 | v2 修订 | 删除 `C` 段（心智模型跨域可用，场景限制扼杀跨域洞察价值）；改用 Qwen 3 系列（`qwen3.6-max-preview` + `qwen3.6-max-preview`）；成本估算从 ¥2-3/场 降到 ¥0.18/场 |
...
| 2026-04-29 | v5.6 模型升级 | `qwen3-max` 系列免费额度全部耗尽（含 max / max-preview / max-2025-09-23 / max-2026-01-23 / qwen-turbo），账户开"仅免费额度"模式后全部 403。整体迁移到 **`qwen3.6-max-preview`**（100 万 token / 2026-07-20 到期）。同时发现 Qwen 3.x 默认开启 reasoning（thinking）阶段，~30s thinking 会被 Vercel edge 60s maxDuration 吃光导致 client 0 chunk —— `api/council.ts` + `api/intake-clarify.ts` 加 `enable_thinking: false` 顶层参数关闭。spec / `.env.example` / Vercel env 全部同步到 `qwen3.6-max-preview` |
